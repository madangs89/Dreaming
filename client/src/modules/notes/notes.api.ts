...
import type { DocumentBody, NoteData, NoteSuccessResponse } from "./notes.type";
...
export const getAllDocuments = async ({ notes_id }: { notes_id: string }) => {
  const response = await axios.get<{
    success: boolean;
    document: DocumentBody[];
  }>(`${API_URL}/api/v1/documents/notes/${notes_id}`, {
    withCredentials: true,
  });
